...
class NormalizerDISC(AbstractNormalizer):
    ...
    def normalize(self, *, limit_low: Optional[Tuple[float, float]] = None,
        ...
        nld_high = transformed.cut(*limit_high, inplace=False)

        popt, cov = self.optimize_CT(nld_high)

        self.res.pars['T'] = [popt[0], np.sqrt(np.diag(cov))[0]]
        self.res.pars['Eshift'] = [popt[1], np.sqrt(np.diag(cov))[1]]

        ext_model = lambda E: self.model(E, T=popt['T'][0],
                                         Eshift=popt['Eshift'][1])
        ext_model_err = lambda E: self.const_temperature_err(E, 
                                                             T=popt[0],
                                                             Eshift=popt[1],
                                                             cov=cov)
        self.res.nld_model = ext_model
        self.res.nld_model_err = ext_model_err
    ...
    def initial_guess(self, limit_low: Optional[Tuple[float, float]] = None,
        ...
        Args:
            limits: The limits (start, stop) where to normalize
               to discrete levels.
        ...
        bounds = [self.bounds['A'], self.bounds['alpha']]
    ...
    def optimize_CT(self, vec):
        
        vec_fit = vec.copy()
        vec_fit.values = np.log(vec_fit.values)
        if vec.std is not None:
            vec_fit.std = vec.std/vec.values
        p, cov = LeastSquares_linear(vec_fit)
        popt = [1/p[0], np.log(p[0])/p[0] - p[1]/p[0]]
        L = np.array([[1/p[0], 0],[np.log(p[0])/p[0], -1]])/p[0]
        cov_trans = cov @ L.T
        cov_trans = L @ cov_trans
        cov=cov_trans

        T = popt[0]
        Terr = np.sqrt(np.diag(cov))[0]
        Eshift = popt[1]
        Eshifterr = np.sqrt(np.diag(cov))[1]

        vals = []
        i = max(0, int(-np.floor(np.log10(Terr))) + 1)
        fmt = '%%.%df' % i
        fmts = '\t'.join([fmt + " ± " + fmt])
        vals.append(fmts % (T, Terr))
        i = max(0, int(-np.floor(np.log10(Eshifterr))) + 1)
        fmt = '%%.%df' % i
        fmts = '\t'.join([fmt + " ± " + fmt])
        vals.append(fmts % (Eshift, Eshifterr))

        self.LOG.info("Model results\n%s", tt.to_string([vals],
            header=['T [MeV]', 'Eshift [MeV]']))

        return np.array([T, Eshift]), cov
    ...
    def plot(self, *, ax: Any = None,
        ...
        nldSn = np.array(self.res.nld_model_err(self.norm_pars.Sn[0]))
        ...
        model, modelerr = self.res.nld_model_err(x)

        ax.plot(x, model, "--", label=labelModel, markersize=0,
                c='g', **kwargs)

        ax.fill_between(x, model-modelerr,
                           model+modelerr, color='g', alpha=0.2)
    ...
    @staticmethod
    def lnlike(x: Tuple[float, float], nld_low: Vector,
               discrete: Vector) -> float:
    ...
    @staticmethod
    def const_temperature_err(E: ndarray, T: float, Eshift: float, cov: ndarray) -> ndarray:
        """ Constant Temperature NLD with errors"""
        ct = np.exp((E - Eshift) / T) / T
        cterr = np.sqrt(cov[0,0]*((E-Eshift)/T + 1)**2 + cov[1,1] + 
                        2*cov[0,1]*((E-Eshift)/T + 1))/T
        return ct, cterr*ct
...
def LeastSquares_linear(vec):
    if vec.std is None:
        w = np.ones(len(vec.values))*(2./len(vec.values))
    else:
        w = 2./vec.std
    
    alpha = np.sum(w*vec.E**2)
    beta = np.sum(w)
    gamma = np.sum(w*vec.E)
    p = np.sum(w*vec.E*vec.values)
    q = np.sum(w*vec.values)
    
    mo = (beta*p - gamma*q)/(alpha*beta - gamma**2)
    co = (alpha*q - gamma*p)/(alpha*beta - gamma**2)
    
    cov = np.array([[beta, -gamma],[-gamma, alpha]])*2/(alpha*beta - gamma)
    
    if vec.std is None:
        S = 1/(len(vec.values) - 1)*sum((mo*vec.E + co - vec.values)**2)
        w = np.ones(len(vec.values))*(2./S)
        alpha = np.sum(w*vec.E**2)
        beta = np.sum(w)
        gamma = np.sum(w*vec.E)
        p = np.sum(w*vec.E*vec.values)
        q = np.sum(w*vec.values)
        cov = np.array([[beta, -gamma],[-gamma, alpha]])*2/(alpha*beta - gamma)
        return np.array([mo, co, S]), cov
    return np.array([mo, co]), cov
