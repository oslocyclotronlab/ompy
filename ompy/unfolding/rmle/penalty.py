        ...
        match self.target:
            case "mu_normalized":
                ...
                def fn(mu, eta, p, axis=None):
                    penalty = onecost(p, threshold, smoothing, axis=axis)
                    return alpha * penalty, penalty
            case "mu":

                def fn(mu, eta, p, axis=None):
                    penalty = onecost(mu, threshold, smoothing, axis=axis)
                    return alpha * penalty, penalty
            case _:
                raise ValueError(
...
def onecost(mu, C, D=100, axis=None):
...
def total_penalty(penalties: tuple[LossFn, ...],
                  ...
                  eta: jnp.ndarray,
                  p: jnp.ndarray,
                  axis: int | None = None) -> tuple[float, float]:
