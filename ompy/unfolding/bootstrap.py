...
from typing import Any, Literal, TypeAlias, overload, TypeVar, Generic
...
def bootstrap_vector_(res: UnfoldedResult1D, N: int, **kwargs) -> BootstrapVector:
    ...
    for i in tqdm(range(N)):
        ...
        if bg is not None:
            ...
            bg_boot = None
        res_ = unfolder.unfold(A_boot, initial=best, R=R, G=G, background=bg_boot, disable_tqdm=True, **kwargs)
    ...
    bootstraped = BootstrapVector(base=res, bootstraps=A_boots, unfolded=unfolded_boot,  # type: ignore
                                  backgrounds=bgs, costs=costs, **kwargs)
    return bootstraped
...
def bootstrap_matrix(res: UnfoldedResult2D, N: int, base: Literal['raw', 'folded'] = 'folded',
    ...
    for i in tqdm(range(N)):
        ...
        initials.append(initial)
    bootstraped = BootstrapMatrix(base=res, bootstraps=A_boots, unfolded=unfolded_boot, kwargs=kwargs, costs=costs,
                                  initials=initials, backgrounds=backgrounds if background is not None else None)
    return bootstraped
...
class Bootstrap(ABC, Generic[T]):
    ...
    def save(self, path: str | Path, exist_ok: bool = False, format: SaveFormat = 'hdf5', **kwargs) -> None:
    ...
    def save_hdf5(self, path: str | Path, exist_ok: bool = False, compression='gzip', **kwargs) -> None:
        ...
        metadata = dict(version=FULLVERSION, base=self.base.__class__.__name__, ndim=self.ndim)
    ...
    def save_npz(self, path: str | Path, exist_ok: bool = False, disable_tqdm: bool = False) -> None:
        ...
        for i in tqdm_(range(len(self.bootstraps))):
            ...
            if self.backgrounds is not None and len(self.backgrounds) > 0:
                self.backgrounds[i].save(path / f"background_{i}.npz",
                                         exist_ok=True)  # np.save(path / f"cost_{i}.npy", self.costs[i])
    ...
    @overload
    @classmethod
    def _load(cls, path: Path, arraytype: type[Matrix], basearray: type[BootstrapMatrix],
              read_only: int | None = None) -> BootstrapMatrix:
        ...

    @overload
    @classmethod
    def _load(cls, path: Path, arraytype: type[Vector], basearray: type[BootstrapVector],
    ...
    def _load_h5(cls, path, arraytype, basearray, read_only):
        ...
        return basearray(base=base, bootstraps=bootstraps, unfolded=unfolded, costs=costs, backgrounds=backgrounds,
                         initials=initials)
    ...
    def memory_usage_rapport(self) -> None:
        g = bytes_to_readable
        memory_usage = {'bootstraps': sum([b.nbytes for b in self.bootstraps]),
...
class BootstrapMatrix(Bootstrap[Matrix]):
    ...
    def eta_ci(self, alpha=0.05, summary=np.median, as_matrix: bool = True, ) -> tuple[Matrix, Matrix] | tuple[
...
def bootstrap_CI(b: Bootstrap, N: int, alpha=0.05, space: VSpace = 'nu') -> tuple[np.ndarray, np.ndarray]:
    """
...
def bootstrap_CI_at(b: Bootstrap, Ex: float | int, N: int, alpha=0.05, space: VSpace = 'nu') -> tuple[
    np.ndarray, np.ndarray]:
    """
    Compute the bootstrap confidence interval for a poisson sample.
