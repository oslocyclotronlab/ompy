...
But this makes the row-normalization condition very tricky.
This code is missing the row-normalization condition.
Use tau trick to ensure non-negativity.

We know we have g=(A,B,alpha) degeneracy. Can we lift them to make the
optimization problem convex?
I don't think so, that is what normalization is for.
How strong is
   - rho is monotone increasing
   - ln rho is linear
   - rho is CT for ef > e_thresh
   - rho fits the discrete levels.
   - T is monotone increasing for some eg > e_thresh
   - rho and T are not "insane" (e.g. rho(Sn) is not 3 or 1e129)
   Maybe not super strong, but it can create priors on (A, B, alpha)
   Maybe only sensible for "canonical" rho and T, e.g. already non-negative.
   Or canoncial in that the ensemble is pre-normalized to a reference rho,T so that
   A, B and alpha are shared between all members of the ensemble.
   Still they probably won't share the same (A, B, alpha) because they are not
   the same. So we have "What (A, B, alpha) normalized *this* pair (rho, T)?" and
   we have "what normalized (rho, T) are consistent with the normalization parameters?"
   and "what distribution of normalization parameters are consistent with the data?"
"""
...
def compute(nld, gsf, Ef, Eg, Ex):
    ...
    return vmap(
        vmap(single_compute, (None, 0, 0)),
        (0, None, None))(
        Ex, Eg, jnp.arange(Eg.shape[0])
    )
