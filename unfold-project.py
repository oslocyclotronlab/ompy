...
def unfold(data_raw, Ex_array, Eg_array, fname_resp_dat, fname_resp_mat, FWHM_factor=10, Ex_min="default", Ex_max="default", Eg_min="default", Eg_max="default", verbose=False, plot=False):
    ...
    if plot:
        ...
        f_proj, ax_proj = plt.subplots(1,1)
        E_proj_low = 7000
        E_proj_high = 7200
        i_proj_low = i_from_E(E_proj_low, Ex_array)
        i_proj_high = i_from_E(E_proj_high, Ex_array)
    
        ax_proj.plot(Eg_array, data_raw[i_proj_low:i_proj_high].sum(axis=0), label="raw")
    ...
    if plot:
        ...
        ax_proj.plot(Eg_array[0:-1], foldmat[i_proj_low:i_proj_high,:].sum(axis=0), label="folded")
    ...
    if plot:
        ...
        ax_proj.plot(Eg_array[0:-1], unfoldmat[i_proj_low:i_proj_high,:].sum(axis=0), label="unfolded")
        ax_proj.plot(Eg_array[0:-1], unfolded[i_proj_low:i_proj_high,:].sum(axis=0), label="compton subtr")
        ax_proj.legend()
