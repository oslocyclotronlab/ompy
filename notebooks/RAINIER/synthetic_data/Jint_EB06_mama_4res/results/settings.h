...
const int g_nEvent = 5e5; // number of events per realization (and ExI in bExSpread)
const int g_nEvUpdate = 1e4; // print progress to screen at this interval
const int g_nEvSave = g_nEvent/10;
...
const double g_dDelta = 3.20713; // effective energy due to pair breaking
  ...
  #ifdef bLDaConst
  const double g_dLDa = 25.16; // MeV^-1 aka "LD parameter a" 
  #endif
  #ifdef bLDaEx
...
const char popFile[] = "240PuPop_combEB06_low.dat"; // made from TALYS "outpopulation y"
// make sure to match # of discrete bins. See ReadPopFile() bins + maxlevelstar + 1 = g_nExPopI
const int g_nExPopI = 83; // bins 0-70; bins + maxlevelstar + 1 = g_nExPopI
