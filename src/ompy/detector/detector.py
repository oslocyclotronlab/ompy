...
BACKEND: Backend = "jax" if _HAS_JAX_WORKING else "numpy"
...
class Detector(ABC, metaclass=CombinedMeta):
    ...
    def resolution_gauss(
        self,
        E: np.ndarray | Index | Array | Vector,
        mu: Unitlike,
    ...
    def resolution_matrix(
        self,
        array: np.ndarray | Vector | Matrix,
        *,
        as_array: Literal[False],
...
class ExDetector(Detector):
    ...
    def _matrix_from_array(
        ...
        if _HAS_JAX_WORKING and isinstance(R, jnp.ndarray):
            R = jnp.transpose(R)
        else:
            R = np.transpose(R)
        matrix = Matrix(X=E, Y=E, values=R, dtype=dtype)
...
class CompoundDetector:
    ...
    def mixture_response(self, Ef: Index, Eg: Index, **kwargs) -> Matrix:
        return mixture_response(Ef, Eg, self, **kwargs)

    def mixture_sigma(self, Ef: Index, Eg: Index, **kwargs) -> Vector:
        return mixture_sigma(Ef, Eg, self, **kwargs)
