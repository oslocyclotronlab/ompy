...
class ResponseData:
    ...
    def normalize_components(
        self, components: Components, inplace: Literal[True] = ...
    ) -> None: ...
    @overload
    def normalize_components(
        self, components: Components, inplace: Literal[False] = ...
    ) -> ResponseData: ...

    def normalize_components(
        self, components: Components, inplace: bool = False
    ) -> ResponseData | None:
    ...
    def normalize(
        ...
        T = (
            compton * self.compton.sum(axis="observed")
            + FE * self.FE
            + SE * self.SE
            + DE * self.DE
            + AP * self.AP
    ...
    @overload
    def normalize_FWHM(
        self, energy: Unitlike, fwhm: Unitlike, inplace: Literal[True] = ...
    ) -> None: ...

    @overload
    def normalize_FWHM(
        self, energy: Unitlike, fwhm: Unitlike, inplace: Literal[False] = ...
    ) -> ResponseData: ...
    ...
    def normalize_FWHM(
        ...
        warnings.warn(
            "You really should not be 'normalizing' FWHM. Do a regression on measured FWHM instead."
        )
    ...
    def from_path(path: Pathlike, **kwargs) -> ResponseData:
        ...
        return ResponseData(
            FE, SE, DE, AP, compton, Eff, FWHM=FWHM[0] if FWHM else None
        )
    ...
    def plot(self, ax: Axes | None = None, **kwargs):
        if ax is None:
            _, ax = plt.subplots(4, 2, constrained_layout=True, sharex=True)
        ax = ax.flatten()
        if len(ax) < 7:
    ...
    def clone(
        ...
        FWHM=None,
        is_normalized=None,
        is_fwhm_normalized=None,
    ) -> ResponseData:
        return ResponseData(
            FE=FE if FE is not None else self.FE,
            ...
            else self.is_normalized,
            is_fwhm_normalized=is_fwhm_normalized
            if is_fwhm_normalized is not None
            else self.is_fwhm_normalized,
        )
