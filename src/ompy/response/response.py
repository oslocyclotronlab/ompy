...
from ..array.index import Index, to_index
...
class Response:
    def __init__(
        ...
        self.interpolation: DiscreteInterpolation = (
            interpolation if not copy else interpolation.clone()
        )
        self.compton: ComptonMatrix | None = (
            compton if not copy else compton.copy() if compton is not None else None
        )
    ...
    def interpolate_compton(
        self, E: Index | np.ndarray | None = None, GPU: bool = True, sigma: float = 6
    ) -> ComptonMatrix:
        if not self.interpolation.is_fwhm_normalized:
            warnings.warn(
                "Interpolating with non-normalized FWHM. Unclear whether this is reasonable."
            )
        ...
        if GPU and not numba_cuda_available():
            raise ValueError(
                "GPU interpolation requested but numba_array cuda not working"
            )
    ...
    def discrete(
        ...
        **kwargs : dict
        ...
        bins_ = self.compton.true_index.handle_rebin_arguments(
            bins=bins, factor=factor, numbins=numbins, binwidth=width
        )
    ...
    def discrete_(
        ...
        *,
        ...
        if self.compton.true_index.leftmost_u > E.leftmost_u and not pad:
            ...
            raise ValueError(
                (
                    "Requested energy grid is too low. "
        ...
        try:
            ...
            raise ValueError(
                "Error rebinning Compton matrix due to bin width.\nThe Compton energy grid is a strict"
        ...
        if pad:
            D_full = Matrix(
                true=E_all,
                observed=E_all,
                values=np.zeros((len(E_all), len(E_all))),
                name="Response",
                xlabel="True energy",
    ...
    def discrete_like(self, other: Matrix | Vector, **kwargs) -> Matrix:
        ...
        **kwargs : dict
        ...
        match other:
            ...
            case _:
                raise ValueError(
                    f"Can only discretize to Matrix or Vector, got {type(other)}"
                )
    ...
    def from_path(cls, path: Pathlike) -> Self:
        ...
        if meta["version"] != FULLVERSION:
            warnings.warn(
                f"Loading response from version {meta['version']} into version {FULLVERSION}."
            )
    ...
    def component_matrices_like(
        self, other: Matrix | Vector | Index, weights: Components | None = None
    ) -> dict[str, Matrix]:
        match other:
            case Matrix():
            ...
            case _:
                raise ValueError(
                    f"Can only discrete to Matrix or Vector, got {type(other)}"
                )

    @overload
    def fold_componentwise(
        self, other: Vector, weights: Components | None = ...
    ) -> FoldedVector: ...

    @overload
    def fold_componentwise(
        self, other: Matrix, weights: Components | None = ...
    ) -> FoldedMatrix: ...

    def fold_componentwise(
        self, other: Matrix | Vector, weights: Components | None = None
    ) -> FoldedVector | FoldedMatrix:
    ...
    def normalize_FWHM(
        self, energy: Unitlike, fwhm: Unitlike, inplace: bool = False
    ) -> Self | None:
        ...
        if inplace:
            ...
            return self.clone(
                interpolation=self.interpolation.normalize_FWHM(
                    energy, fwhm, inplace=inplace
                )
            )
    ...
    def normalize_sigma(
        ...
        if inplace:
            ...
            return self.clone(
                interpolation=self.interpolation.normalize_sigma(
                    energy, sigma, inplace=inplace
                )
            )
...
def gaussian_matrix(E: np.ndarray, sigmafn) -> Matrix:
    ...
    return Matrix(
        ...
        observed=E,
        values=values,
        ylabel=r"Measured $E_\gamma$",
        xlabel=r"True $E_\gamma$",
        edge="mid",
    )
