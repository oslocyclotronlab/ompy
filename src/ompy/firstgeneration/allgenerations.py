...
def compute_ag_chunked(FG, Ex, Ef_map, chunk_size=100):
    ...
    def process_chunk(start_idx, end_idx, AG):
        ...
        return jax.lax.scan(update_ag_single, AG, jnp.arange(start_idx, end_idx))[0]
...
def optimize(
    ...
    G_g: Matrix | None = None,
    G_in: Matrix | None = None,
    D: Matrix | None = None,
    pop=None,
    mask: np.ndarray | Matrix | Literal["tril"] = "diagonal",
    ...
    def scan_step(carry, i):
        ...
        if record_metrics:
            ...
            metrics = (loss, grad_norm, row_norms, col_norms, grads[1])
        else:
            metrics = loss
        return (params, opt_state), metrics

    (params, opt_state), metrics = jax.lax.scan(
        scan_tqdm(n=iterations)(scan_step),
        (params, opt_state),
        jnp.arange(iterations),
        length=iterations,
    )
    if record_metrics:
        losses, grad_norms, row_grad_norms, col_grad_norms, censor_norms = metrics
    else:
        losses = metrics
    ...
    losses = Vector(index=np.arange(iterations), values=losses)
    return AGResult(
        AG=Y,
        FG=fg_hat,
        optimizer=optimizer,
