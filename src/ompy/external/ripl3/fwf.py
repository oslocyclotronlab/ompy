...
def parse_fwf(
    string: str, fmt: str, space_as_delim: bool = False, missing: str | None = None
) -> Iterator[Any]:
    ...
    missing : str | None, optional
        Characters that represent missing values. If None, missing values
        will raise an exception as before. If a string (e.g., '-'), fields
        containing only those characters will be converted to None.
    ...
    >>> list(parse_fwf(" 12  -   3.14", "i3, a6, f4.2", missing="-"))
    (12, None, 3.14)
    """
    ...
    for i, field in enumerate(fields):
        try:
            ...
            value, string = field.parse(string, scale=scale, missing=missing)
            ...
            if (
                missing is not None
                and _head.strip()
                and all(c in missing for c in _head.strip())
            ):
                yield None
                string = _body
                continue
                ...
                raise FwfParseError(
                    f'Error converting "{_head}" using {field}:\n'
                    f"{highlight_error(string, 0, field.width)}"
                ) from e
        except Exception as e:
            raise FwfParseError(
                f"Error parsing field {i}: {field} of string `{string}`"
            ) from e
...
def highlight_error(input_string, error_position, error_length=1):
    ...
    truncated = (
        (input_string[: 70 - 3] + "...") if len(input_string) > 70 else input_string
    )
...
class Field:
    ...
    def parse(
        self, string: str, scale: float = 1.0, missing: str | None = None
    ) -> tuple[Any, str]:
        head, body = self.split(string)
        # Check if the field contains only missing characters
        if missing is not None and (
            not head.strip() or all(c in missing for c in head.strip())
        ):
            return None, body
...
class NumericField(Field):
    def parse(
        self, string: str, scale: float = 0.0, missing: str | None = None
    ) -> tuple[Any, str]:
        head, body = self.split(string)
        # Check if the field contains only missing characters
        if missing is not None and (
...
class Scale(Field):
    ...
    def parse(
        ...
        if self.spec is None:
            return "", string
        return self.spec.parse(string, self.scale, missing)
...
def parse_fwf_format(fmt: str) -> Iterator[Field]:
    ...
    >>> list(parse_fwf_format("2(e8.2,2x)"))
    [ScientificField(type=<class 'float'>, width=8, exponent=2), SpaceField(type=<class 'str'>, width=2), ScientificField(type=<class 'float'>, width=8, exponent=2), SpaceField(type=<class 'str'>, width=2)]
    """
    # First, we need to handle the format string more carefully to account for parentheses
    fields = split_format_string(fmt)

    for field in fields:
        field = field.strip()

        # Check if this is a repeated group like "2(e8.2,2x)"
        if "(" in field and ")" in field:
            # Extract the repeat count and the group format
            paren_start = field.find("(")
            paren_end = field.rfind(")")

            if paren_start == 0:
                # No repeat count, just a group
                group_fmt = field[1:paren_end]
                repeats = 1
            else:
                # Has repeat count
                repeat_str = field[:paren_start]
                group_fmt = field[paren_start + 1 : paren_end]
                repeats = int(repeat_str) if repeat_str else 1

            # Parse the group format recursively
            group_fields = list(parse_fwf_format(group_fmt))
            ...
            repeats, type_fmt, body = split_field_spec(field)
            match type_fmt:
                case "i":
...
def split_format_string(fmt: str) -> list[str]:
    """Split a format string into fields, handling parentheses correctly.

    Parameters
    ----------
    fmt : str
        The format string to split
