...
if jax_working():
    import jax
    from jax import Array as ArrayImpl # public array type
    Device: TypeAlias = Literal['cpu'] | jax.Device
else:
...
class AbstractArray(AbstractArrayProtocol[NDArray[Any], Any], ABC):
    ...
    def __array_function__(self, func: Any, types: tuple[type, ...], args: tuple[Any, ...], kwargs: dict[str, Any]) -> Any:
        """Implement the __array_function__ protocol for NumPy functions.
        
        This allows functions like np.cumsum, np.sum, np.mean, etc. to return
        AbstractArray instances instead of plain numpy arrays.
        """
        # Handle np.stack specifically to avoid recursion while providing functionality
        if func is np.stack:
            # Extract the underlying numpy arrays
            arrays = []
            for arg in args[0]:  # args[0] is the sequence of arrays
                if isinstance(arg, AbstractArray):
                    arrays.append(arg.values)
                else:
                    arrays.append(arg)
            
            # Call np.stack on the underlying arrays
            result = np.stack(arrays, **kwargs)
            return result
        
        # Skip other functions that should not be handled to avoid recursion
        # These functions work better with the default numpy behavior
        skip_functions = {
            np.concatenate, np.vstack, np.hstack, np.dstack,
            np.column_stack, np.row_stack, np.block, np.array_split,
        ...
        # Check if all types can be handled - use a safer approach to avoid recursion
        try:
            for t in types:
                if not (t is AbstractArray or 
                        t is np.ndarray or 
                        t in (int, float, complex) or
                        (hasattr(t, '__mro__') and AbstractArray in t.__mro__)):
                    return NotImplemented
        except (TypeError, RecursionError):
            # If there's any issue with type checking, fall back to NotImplemented
            return NotImplemented
        
        # Extract the underlying numpy array from AbstractArray instances
        new_args = []
        for arg in args:
            if isinstance(arg, AbstractArray):
        ...
        # Call the numpy function on the underlying arrays
        result = func(*new_args, **kwargs)
        
        # Wrap the result back in the appropriate class if it's an array
        if isinstance(result, np.ndarray):
            # Find the first AbstractArray in args to use as a template
            template = next((arg for arg in args if isinstance(arg, AbstractArray)), None)
            if template is not None and result.shape == template.values.shape:
                return template.clone(values=result)
            # If shape changed or no template, return numpy array
            # (e.g., for reduction operations that change shape)
            return result
        else:
            # Scalar result (e.g., sum, mean with no axis specified)
            return result
    ...
    def _coerce_other(self, other: AbstractArray | NDArray[Any] | float):
        """Default: if other is AbstractArray, require exact shape + bin-compat (legacy).
        Subclasses (Vector/Matrix) should override to allow broadcasting with index checks.
        """
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(f"Neither {name} nor {name}.values has '{attr}'")
    ...
    @property
    def vunit(self) -> Unit:
        """Unit for the values (y-axis for Vector, z-axis/colorbar for Matrix)"""
        return self.metadata.vunit
