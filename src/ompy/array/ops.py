...
if jax_working():
    ...
    def subtract_background(
        total: np.ndarray | Matrix,
        bg: np.ndarray | Matrix,
        n_iterations: int = 1000,
        optimizer: optax.GradientTransformation = optax.adam(1e-3),
    ) -> Tuple[np.ndarray | Matrix, Vector]:
        """
        Recover foreground from total = fg + bg using Poisson MLE with optax.
        
        Args:
            total: Observed total counts (2D array)
            bg: Known background counts (2D array)
            learning_rate: Learning rate for optimizer
            n_iterations: Number of optimization iterations
            verbose: Whether to print progress
        
        Returns:
            fg: Recovered foreground (2D array)
            loss_history: List of loss values during optimization
        """
        template = None
        if isinstance(total, AbstractArray):
            template = total
            total = total.values
        if isinstance(bg, AbstractArray):
            if template is None:
                template = bg
            bg = bg.values

        # Convert to JAX arrays
        total_jax = jnp.array(total, dtype=jnp.float32)
        bg_jax = jnp.array(bg, dtype=jnp.float32)
        
        # Initialize fg as a simple estimate (total - bg, clipped to non-negative)
        fg_init = jnp.maximum(total_jax - bg_jax, 0.0)
        fg_init = jnp.where(fg_init < 1e-2, 1e-2, fg_init)
        fg_init = jnp.log(fg_init)
        # Check that fg_init is finite
        
        # Setup optimizer
        opt_state = optimizer.init(fg_init)
        
        # Define loss function and its gradient
        @jax.jit
        def loss_fn(fg):
            fg = jnp.exp(fg)
            #loss = kl_div(fg + bg_jax, total_jax)
            nu = fg + bg_jax
            n = total_jax
            eps = 1e-10
            loss =nu - n + n * jnp.log(n / (nu + eps) + eps)
            return loss.sum()
            #return poisson_nll_loss(fg, total_jax, bg_jax)
        
        @jax.jit
        def update_step(fg, opt_state):
            loss, grads = jax.value_and_grad(loss_fn)(fg)
            updates, opt_state = optimizer.update(grads, opt_state)
            fg = optax.apply_updates(fg, updates)
            return fg, opt_state, loss
        
        # Optimization loop
        fg = fg_init
        loss_history = np.empty(n_iterations)
        
        @scan_tqdm(n_iterations)
        def scan_body(carry, _):
            fg, opt_state = carry
            fg, opt_state, loss = update_step(fg, opt_state)
            return (fg, opt_state), loss
        
        init_carry = (fg, opt_state)
        (fg, opt_state), loss_history = jax.lax.scan(scan_body, init_carry, jnp.arange(n_iterations))
        
        # Ensure final result is non-negative
        fg_final = jnp.exp(fg)
        if template is not None:
            fg_final = template.clone(values=fg_final, name='foreground')

        loss = Vector(X=np.arange(n_iterations), values=loss_history, name='loss')
        
        return fg_final, loss