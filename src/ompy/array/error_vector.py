...
from .ops import unpack_to_vectors
...
class AsymmetricVector(ErrorVector):
    ...
    def add_error(
        ...
        return cls(X=other.X_index, values=other.values, lerr=lerr, uerr=uerr, metadata=other.metadata, **kwargs)
    ...
    def from_list(
        ...
        return cls.from_CI(vec, lerr, uerr, metadata=vectors[0].metadata, **kwargs)
    ...
    def strip(self) -> Vector:
        return Vector(X=self.X_index, values=self.values, metadata=self.metadata)
