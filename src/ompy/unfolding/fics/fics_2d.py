...
from ...stubs import Axes, array2D, array3D, Plot1D
from ..result2d import Cost2D, UnfoldedResult2D
from .fics_1d import chi2_safe

try:
    from ..utils import loop_tqdm
except Exception:  # pragma: no cover - pure fallback for optional deps
...
if _jax_ready:
    ...
    if not _JAX_AVAILABLE:
        _JAX_ERROR = JAXNotInstalledError(
            "Install `jax` to use the accelerated FICS 2D routines."
        )
    else:
    ...
    def unfold_matrix_jax(*args, **kwargs):
        _raise_jax_error()

    def unfold_matrix_jax_block(*args, **kwargs):
        _raise_jax_error()

    def kl_jax(*args, **kwargs):
        _raise_jax_error()

    def chi2_jax(*args, **kwargs):
        _raise_jax_error()
