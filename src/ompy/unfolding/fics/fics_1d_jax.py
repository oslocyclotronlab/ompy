...
jax, jnp, _jax_ready, _JAX_AVAILABLE = get_jax(
    require_working=True,
    allow_fallback=False,
    prefer_accel=_HAS_JAX_WORKING,
)

_JAX_READY = bool(_jax_ready and _HAS_JAX_WORKING)
_JAX_ERROR = None

if _JAX_READY:
    try:
        from jax_tqdm import scan_tqdm
    except Exception:  # pragma: no cover - optional progress helper

        def scan_tqdm(iterable=None, **kwargs):
            def decorator(func):
                return func
    ...
    def unfold_vectors(
        ...
        batched_unfold = jax.vmap(
            ...
            in_axes=(
                0,
                0,
            ),  # Vectorize over first argument (raw) and second argument (initial)
    ...
    def gaussian_smooth(
        x: jnp.ndarray, sigma: float, radius: int, axis: int = -1
    ) -> jnp.ndarray:
        """
        ...
        smoothed = jnp.apply_along_axis(
            lambda row: jnp.convolve(row, kernel, mode="same"), axis=-1, arr=x_t
        )
    ...
    if not _JAX_AVAILABLE:
        _JAX_ERROR = JAXNotInstalledError(
            "Install `jax` to use the accelerated FICS 1D routines."
        )
    elif not _jax_ready:
        _JAX_ERROR = JAXNotWorkingError(
            "JAX is installed but no suitable accelerator backend was detected."
        )
    else:
        _JAX_ERROR = JAXNotWorkingError(
            "JAX devices appear available, but accelerator readiness probe failed."
        )

    def scan_tqdm(iterable=None, **kwargs):  # type: ignore[override]
        def decorator(func):
            return func

        return decorator

    def _raise_jax_error(*_args, **_kwargs):
        assert _JAX_ERROR is not None
        raise _JAX_ERROR

    def unfold_vector(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()

    def unfold_vectors(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()

    def chi2(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()

    def chi2_safe_1d(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()

    def chi2_safe(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()

    def kl(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()

    def fluctuation_cost(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()

    def gaussian_smooth(*args, **kwargs):  # type: ignore[override]
        _raise_jax_error()


# Convenience function to convert numpy arrays to JAX and back
def _ensure_jax_ready():
    if _JAX_ERROR is not None:
        raise _JAX_ERROR
    if not _JAX_READY:
        raise JAXNotWorkingError(
            "JAX backend is disabled for FICS 1D routines (accelerator unavailable)."
        )
...
def unfold_vector_wrapper(
    ...
    # Run JAX function
    u_all, cost, fluctuations, kl_cost = unfold_vector(R, raw, initial, iterations, lr)

    # Convert back to numpy
    return (np.array(u_all), np.array(cost), np.array(fluctuations), np.array(kl_cost))


def unfold_vectors_wrapper(
    R: array1D,
    raw_list: list[array1D],
    initial_list: list[array1D],
    iterations: int,
    lr: float,
):
